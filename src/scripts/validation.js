...
const setErrorMessage = (input, message) => {//функция установки кастомного сообщения об ошибке
  input.setCustomValidity(message);
};
const showErrorMessage = (input, classes, errorElement) => {//показать сообщение об ошибке валидации
  errorElement.textContent = input.validationMessage;
  errorElement.classList.add(classes.errorClass);
  input.classList.add(classes.inputErrorClass);
};
const hideErrorMessage = (input, classes, errorElement) => {//скрыть сообщение об ошибке валидации
...
const isValidInput = (input) => { //вывод валидности конкретного инпута
...
const validateInput = (input, classes, errorElement) => { //непосредственно валидация инпута с выводом ошибок
...
const isValidForm = (elementsArray) => { //вывод валидности формы
...
const setFormListeners = (form, classes) => { //назначить слушатели всем формам
